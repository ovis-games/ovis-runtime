...
bool foo_iterator(void**, void** outputs) {
    EntityDescriptorList->initialize(EntityDescriptorList, outputs[0]);
    ...
    TYPE_FUNCTION(TYPE(ovis, runtime, List), add)(EntityDescriptorList, spawn_list, &entity_desc_storage);
...
bool foo1(struct Scene* scene) {
    const int32_t components[] = {EntityDescriptorList_resource_id};
